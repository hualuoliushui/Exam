...
namespace detail_range {
	...
	public:
		iterator_impl(value_type begin_val, value_type end_val, value_type step_val)
			:begin_(begin_val)
			, end_(end_val)
			, step_(step_val)
	...
	template<typename T>
	detail_range::iterator_impl<T> range(T end) { return { {},end,1 }; }

	template<typename T>
	detail_range::iterator_impl<T> range(T begin, T end) { return { begin,end,1 }; }

	template<typename T,typename U>
	auto range(T begin, T end, U step)
		->detail_range::iterator_impl<decltype(begin + step)>
	{
		// may be narrowing
		using r_t = detail_range::iterator_impl<decltype(begin + step)>;
		return r_t(beging, end, step);
	}
};